...
export default function Product() {
  ...
  return (
    <div>
      <div style={{ backgroundColor: "#BFD4E4", padding: 16, display: "flex", justifyContent: "center" }}>
        <input
          type="text"
          placeholder="Search products..."
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          style={{ padding: 8, width: 380, color: "white", backgroundColor: "#042E61", border: "none", borderRadius: 50 }}
        />
      </div>
      ...
      {filteredProducts.length === 0 ? (
        ...
        <Row gutter={[16, 16]} style={{ backgroundColor: "#BFD4E4" }}>
          {filteredProducts.map((product) => (
            <Col xs={10} sm={10} md={10} lg={8} key={product.id} style={{ backgroundColor: "#BFD4E4", padding: 32 }}>
              <Card hoverable style={{ backgroundColor: "#40648F", color: "white" }}>
                <div style={{ backgroundColor: "white", height: "273px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {product.image && <img src={product.image} alt={product.name?.[language]} style={{ height: "273px" }} />}
                </div>
                <h1 style={{ fontFamily: "Cormorant Garamond, serif", fontSize: 20 }}>{product.name?.[language]}</h1>
                <p> Rp {product.price}</p>
                <Button
                  type="primary"
                  style={{ backgroundColor: "#BFCDE4", borderRadius: 50, border: "none", fontSize: 20, width: "100%", height: "40px", marginTop: "10px", color: "black" }}
